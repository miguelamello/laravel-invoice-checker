...
class InvoiceController extends Controller
{
    ...
    public function getInvoice($id)
    {
        ...
        if ($invoice) {
         ...
         $invoice['products'] = Products::find($invoice->company_id);         
        }
    ...
    public function getInvoiceProducts($invoice_id, $product_id)
    {

        //Sanitize $invoice_id and $product_id
        $sanitized_invoice_id = trim($sanitized_invoice_id);
        $sanitized_invoice_id = strlen($sanitized_invoice_id);
        $sanitized_product_id = trim($sanitized_product_id);
        $sanitized_product_id = strlen($sanitized_product_id);

        //Check if ids has a valid UUID length
        if ($sanitized_invoice_id !== 36 || $sanitized_product_id !== 36) {
            //Return empty array if not valid ids
            return [];            
        }

        //Return Invoice
        $invoice = Invoices::find($sanitized_invoice_id);

        if ($invoice) {
         $invoice['company'] = Companies::find($invoice->company_id);
         $invoice['products'] = InvoiceProducts::find($invoice->company_id);         
        }
